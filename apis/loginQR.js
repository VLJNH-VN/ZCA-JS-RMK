...
import { logger, request, generateZaloUUID } from "../utils.js";
...
export async function loginQR(ctxOrOptions, optionsOrCallback, callbackArg) {
    let ctx, options, callback;
    if (ctxOrOptions && (ctxOrOptions.API_TYPE !== undefined || ctxOrOptions.secretKey !== undefined)) {
        ctx = ctxOrOptions;
        options = optionsOrCallback || {};
        callback = callbackArg;
    } else {
        const { appContext: _appCtx } = await import("../context.js");
        ctx = _appCtx;
        options = ctxOrOptions || {};
        callback = optionsOrCallback;
    }
    ctx.cookie = new CookieJar();
    ctx.userAgent = options.userAgent || "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36";
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // Đảm bảo ctx.imei tồn tại (cần cho bước login sau)
            if (!ctx.imei) {
