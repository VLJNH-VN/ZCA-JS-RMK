...
function resolveCtx(ctxOrEncrypt) {
    if (typeof ctxOrEncrypt === "boolean" || ctxOrEncrypt === undefined || ctxOrEncrypt === null) {
        return { ctx: appContext, encryptParams: ctxOrEncrypt };
    }
    return { ctx: ctxOrEncrypt, encryptParams: undefined };
}
export async function login(ctxOrEncrypt, encryptParamsArg) {
    const { ctx, encryptParams: ep } = resolveCtx(ctxOrEncrypt);
    const encryptParams = encryptParamsArg !== undefined ? encryptParamsArg : ep;
...
export async function getServerInfo(ctxOrEncrypt, encryptParamsArg) {
    const { ctx, encryptParams: ep } = resolveCtx(ctxOrEncrypt);
    const encryptParams = encryptParamsArg !== undefined ? encryptParamsArg : ep;
