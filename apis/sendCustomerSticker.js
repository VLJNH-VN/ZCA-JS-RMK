...
export function sendCustomStickerFactory(api) {
   ...
   * @param {boolean} [ai=false] Đánh dấu là AI sticker (jcp.pStickerType=1)
   * @throws {ZaloApiError}
   */
  return async function sendCustomSticker(message, staticImgUrl, animationImgUrl, width = null, height = null, ttl = 0, ai = false) {
    ...
    if (typeof message === "object" && !message.data && message.staticImgUrl) {
      ...
      type = message.type ?? message.threadType ?? MessageType.DirectMessage;
    ...
    if (ai) {
      params.jcp = JSON.stringify({ pStickerType: 1 });
    }
